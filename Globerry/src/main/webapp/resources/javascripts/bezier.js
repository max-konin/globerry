...
function Graph(w, h, _minX, _maxX, _minY, _maxY, step, lmap) {
	...
	var canvas = graphForBezier(lmap);
	...
	function redrawSpans(points, zLevel) {		
		canvas.removeAll();
		var point;
		var pointsToDraw = [];
		for(var j = 0; j < points.length; j++) {
			point = new Point(points[j].longitude, points[j].latitude);
			point.weight = points[j].weight;
			pointsToDraw.push(point);
		}
		drawSpans(pointsToDraw, zLevel);
	}
			
	function drawSpans(points, zLevel) {
		function Z(x, y) {
			var val = 0;
			for(var i = 0, l = points.length; i < l; i++) {
				var point = points[i];
		...
		var stepX = 0.15, stepY = 0.15, eps = 0.1, spanFactor = 3;
		if(points.length == 0)
			return;
		var pointsRemaining = [];
		...
		for(var i = 0; pointsRemaining.length > 0; i++) {
			var firstPoint = pointsRemaining[0];
			pointsRemaining.splice(0,1);
			var y = firstPoint.y;
			var p = Point(firstPoint.x + 0.02, firstPoint.y);
			...
			while(spanStack.size() > 0) {
				...
				for(var j = 0; j < spans.length; j++)
					spanStack.push(spans[j]);
			}
			p = firstSpan.getLeft();
			var polygon = [];
			for(var t = 0; t < 1000 ; t++) {
				polygon.push(new L.LatLng(p.y, p.x));
				p = p.next;
			...
			var tPolygon = new L.ExPolygon(polygon,{});
			for(var tr = 0; tr < pointsRemaining.length; tr++) {
				if(tPolygon.pointInPolygon(pointsRemaining[tr])) {
					pointsRemaining.splice(tr, 1);
					tr--;
				}
			}
			canvas.putBezier(i, polygon);			
		}
	}
	var me = {
		svg : svg,
...
function ZTile(_x, _y, _sizeX, _sizeY, foo) {
	...
	function setSiseX(size) {
		_sizeX = size;
	}
	function getSizeY() {
		return sizeY;
	}
	function setSiseY(size) {
		_sizeY = size
		}
	function getCorner() {
		return corner;
...
L.ExPolygon = L.Polygon.extend({
	...
	pointInPolygon : function (/*Point(x,y)*/point) {
		...
		var influence = .05;
		var x0 = point.x, y0 = point.y, x1, x2, y1, y2, angle = 0;
